    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-8 overflow-y-auto">
          ...
          <div className={cn("relative z-10 w-full rounded-2xl border border-graphite-200 bg-white shadow-pop animate-fadeUp my-auto", wide ? "max-w-2xl" : "max-w-md")}>
            <div className="flex items-start justify-between p-5 pb-3">
              <div>
                <h3 className="text-base font-semibold text-navy">{title}</h3>
                {description && <p className="mt-0.5 text-[13px] text-graphite-500">{description}</p>}
              </div>
              <button onClick={close} className="h-8 w-8 grid place-items-center rounded-lg text-graphite-400 hover:bg-graphite-100">
