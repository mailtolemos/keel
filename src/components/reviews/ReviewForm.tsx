...
export function ReviewForm({ responseId, kind, subjectName, questions, ratingScale, initial, close }: {
  ...
  return (
    <div className="space-y-5 max-h-[72vh] overflow-y-auto pr-1">
      <p className="text-[13px] text-graphite-600">{isManager ? `Manager review for ${subjectName}` : "Your self review"}</p>
      {questions.map((q, i) => {
          ...
          <div key={q.id}>
            ...
            {ratingScale && q.hasRating && (
              <div className="mt-2 flex items-center gap-1.5">
                <span className="text-[12px] text-graphite-500 mr-1">Rating</span>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} type="button" onClick={() => setAns(q.id, { rating: n })} className={cn("h-7 w-7 rounded-md border text-[12px] font-medium", a.rating === n ? "border-accent bg-accent text-white" : "border-graphite-200 text-graphite-500 hover:bg-graphite-50")}>{n}</button>
                ))}
              </div>
            )}
          </div>
      ...
      {ratingScale && (
        <div>
          ...
          <div className="flex items-center gap-1.5">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} type="button" onClick={() => setOverallRating(n)} className={cn("h-9 w-9 rounded-lg border text-sm font-medium", overallRating === n ? "border-accent bg-accent text-white" : "border-graphite-200 text-graphite-500 hover:bg-graphite-50")}>{n}</button>
            ))}
            <span className="ml-2 text-[12px] text-graphite-400">1 = needs focus · 5 = exceptional</span>
          </div>
        </div>
      )}
