...
export default async function ReviewsPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div>
      ...
      {myOpen.length > 0 && (
        <Card className="p-5 mb-5 border-accent-100 bg-accent-soft">
          <h2 className="text-sm font-semibold text-ink mb-3">Your open reviews</h2>
          <div className="space-y-2.5">
            {myOpen.map((r) => (
              <div key={r.id} className="flex items-center gap-3 rounded-lg bg-surface border border-line px-3 py-2.5">
                <div className="h-8 w-8 rounded-lg bg-accent-soft grid place-items-center"><Icon.reviews size={16} className="text-ink" /></div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] font-medium text-ink">{r.kind === "SELF" ? "Self review" : `Review of ${r.subject.name}`}</p>
                  <p className="text-[12px] text-ink-soft">{r.cycle.name}{r.cycle.dueDate ? ` · due ${fmtDate(r.cycle.dueDate)}` : ""}</p>
                </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {cycles.length === 0 && <Card className="p-10 text-center text-[13px] text-ink-soft md:col-span-2">No review cycles yet.{perms.isAdmin ? " Create your first one." : ""}</Card>}
        {cycles.map((c) => {
            ...
            <Link key={c.id} href={`/${slug}/reviews/${c.id}`}>
              <Card className="p-5 hover:border-accent-100 hover:shadow-card transition h-full">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] text-ink-faint">{c.participants.length} participants</span>
                </div>
                <h3 className="font-semibold text-ink mt-2.5">{c.name}</h3>
                <p className="text-[12px] text-ink-soft mt-0.5">{fmtDate(c.periodStart)} – {fmtDate(c.periodEnd)}{c.dueDate ? ` · due ${fmtDate(c.dueDate)}` : ""}</p>
                <div className="mt-4">
                  <div className="flex justify-between text-[12px] mb-1"><span className="text-ink-soft">Completion</span><span className="font-medium text-ink">{pct}%</span></div>
